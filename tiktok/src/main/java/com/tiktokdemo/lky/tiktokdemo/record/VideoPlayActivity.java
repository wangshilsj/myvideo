...
public class VideoPlayActivity extends Activity implements View.OnClickListener, VideoPlayContract.View {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tidal_pat_record_voice_complete_btn) {
            ...
            mPresenter.complete();
        } else if (v.getId() == R.id.tidal_pat_record_video_back_img) {
            onBackPressed();
            ...
            showCutAudioLayout();
        } else if (v.getId() == R.id.tidal_pat_upload_video_volume_img) {
            ...
            showVolumeLayout();
        } else if (v.getId() == R.id.tidal_pat_record_volume_layout) {
            hideVolumeLayout();
        } else if (v.getId() == R.id.tidal_pat_upload_video_special_effects_img) {
            if (mTidalPatRecordDraftBean.getRecordTimeType() != RecordTimeType.RECORD_TIME_15) {
                return;
            }
            inSpecialEffectsMode();
        } else if (v.getId() == R.id.personal_show_record_video_loading_layout) {
...
class SpecialEffectsViewClickListener implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (v.getId() ==R.id.tidal_pat_upload_se_bottom_filter_txt){
            ...
            selectedFilterMode();
        }else if (v.getId() == R.id.tidal_pat_upload_se_bottom_time_txt){
            if (mPresenter.getSpecialEffectsParentType() == SpecialEffectsParentType.TIME) {
                return;
            }
            mPresenter.changeSpecialEffectsMode(SpecialEffectsParentType.TIME);
            selectedTimeMode();
        }else if (v.getId() ==R.id.tidal_pat_upload_se_save_txt ){
            outSpecialEffectsMode(true);
        }else if (v.getId() == R.id.tidal_pat_upload_se_cancel_txt ){
            outSpecialEffectsMode(true);
        }else if (v.getId() == R.id.tidal_pat_record_video_upload_pv){
            if (isSpecialEffectsEditMode) {
                mVideoPlayView.changeState();
            }
        }else if (v.getId() == R.id.tidal_pat_upload_se_remove_txt){
